   ...
   if(timeFrame!=Period())
     {
      ...
      if(trend[limit]==-1)
         CleanPoint(limit,ddaBuffer,ddbBuffer);
      for(int i=limit; i>=0; i--)
        {
         ...
         if(Interpolate<1 || y==iBarShift(NULL,timeFrame,Time[i-1]))
            continue;
      ...
      for(i=limit; i>=0; i--)
         if(trend[i]==-1)
            PlotPoint(i,ddaBuffer,ddbBuffer,dssBuffer);
      return(0);
     }
   ...
   for(i=limit, r=Bars-i-1; i>=0; i--,r++)
     {
      ...
      //Print("momc: ",momc*1000," momh: ",momh*1000, " moml: ",moml*1000);

      dssBuffer[i] = iDss(momc,momh,moml,StochasticLength,SmoothMA,SmoothMAMethod,i);
      //Print("dssBuffer: ",dssBuffer[i]);
      ddaBuffer[i] = EMPTY_VALUE;
      ddbBuffer[i] = EMPTY_VALUE;
...
double iDss(double pClose, double pHigh, double pLow, int period, int smoothPeriod, int smoothMethod, int i, int instanceNo=0)
  {
   if(ArrayRange(workDss,0)!=Bars)
      ArrayResize(workDss,Bars);
   i = Bars-i-1;
   instanceNo*=5;
//Print("instanceNo: ",instanceNo);
...
//Print("workDss[i][instanceNo+_phigh]: ",workDss[i][instanceNo+_phigh]*1000,
//", [i][instanceNo+_plow]: ",workDss[i][instanceNo+_plow]*1000,", min: ",min*1000,", max: ",max*1000);
...
//Print("min: ",min*10000,", max: ",max*10000);
   workDss[i][instanceNo+_st1] = 0;
   if(min!=max)
      workDss[i][instanceNo+_st1] = 100*(pClose-min)/(max-min);
//Print("workDss[i][instanceNo+_st1]: ",100*(pClose-min)/(max-min));
   ...
   if(i==0)
     {
      ...
      //   Print("workDss[i][instanceNo+_ss1]: ",workDss[i][instanceNo+_ss1],", period: ",period,
      //", min: ",min*1000,", max: ",max*1000," (max-min): ",(max-min)*100);
     }
   else
     {
      //Print(Bars-i-1);
      //Print("workDss[i][instanceNo+_st1] ::> ",workDss[i][instanceNo+_st1]);
      ...
      double stoch = 0;
      if(min!=max)
         stoch = 100*(workDss[i][instanceNo+_ss1]-min)/(max-min);
      //Print("stoch ::> ",stoch);
      workDss[i][instanceNo+_dss] = iCustomMa(smoothMethod,stoch,smoothPeriod,Bars-i-1,instanceNo*2+1);
      //Print("_dss::> ",workDss[i][instanceNo+_dss],", _ss1 ::> ",workDss[i][instanceNo+_ss1]," -- ",TimeToStr(Time[i]));
     }
   Print("RETVAL ::> ",MathMin(MathMax(workDss[i][instanceNo+_dss],0),100));
//
//
//
//
...
//Print(i,"-->",r);
   switch(mode)
     {
      case 0  :
         return(iSma(price,length,r,instanceNo));
      case 1  :
      ...
      case 12 :
         return(iSineWMA(price,length,r,instanceNo));
      case 13 :
      ...
      case 19 :
         return(iSmooth(price,length,r,instanceNo));
      default :
         return(0);
     }
  }
...
//Print("sumw: ",sumw,", sum: ",sum," price: ",price);
   for(int k=1; k<period && (r-k)>=0; k++)
     {
      double weight = period-k;
      //Print("weight: ",weight,", k: ",k," r: ",r," r-k: ",r-k);
      sumw  += weight;
...
//if(instanceNo==0)
//   Print("PRE CICLO ::::> sumw: ",sumw,", sum: ",sum," price: ",price);
   ...
   for(int k=1; k<period && (r-k)>=0; k++)
     {
      ...
      //if(instanceNo==0)
      //   Print("PRE: weight: ",weight,", k: ",k," r: ",r," r-k: ",r-k," sumw: ",sumw," sum: ",sum," workLwmp[r-k][instanceNo]: ",workLwmp[r-k][instanceNo]);
      sumw  += weight;
      sum   += weight*workLwmp[r-k][instanceNo];
      //if(instanceNo==0)
      //   Print("POST: weight: ",weight,", k: ",k," r: ",r," r-k: ",r-k," sumw: ",sumw," sum: ",sum," workLwmp[r-k][instanceNo]: ",workLwmp[r-k][instanceNo]);
     }
//if(instanceNo==0)
//   Print("FINAL CICLO ::::> sum: ",sum," sumw: ",sumw);
   return(sum/sumw);
  }
...
//
   ...
   if(nlmvalues[instanceNo][_weight]>0)
     {
      double sum = 0;
      for(k=0; k < nlmvalues[instanceNo][_len]; k++)
