      ...
      for(i=limit; i>=0; i--)
        {
         ...
         //Print("vprice: ",vprice,", cprice: ",cprice," ",vprice==cprice);
         double upValue;
         double dnValue;
         double miValue;

         //Print("upValue: ",upValue,", dnValue: ",dnValue,", miValue: ",miValue);
         ...
         if(alertsOnZeroCross)
         ...
         else
           {
            ...
            if(alertsOnAllBreakOuts || alertsOnAllBreakOutBars)
               if(cprice<upValue && cprice>dnValue)
                  trend[i] =  0;
           }
         manageArrow(i);
        }
...
//if(r<=25)
//    {
//     Print("hprice: ",hprice*100,", lprice: ",lprice*100,", wrk[r][vprice]: ",wrk[r][vprice]);
//    }
...
//Print("len1: ",len1,", pow1: ",pow1,", r: ",r);
   ...
   double avg = wrk[r][vsum];
   for(k=1; k<avgLen && (r-k)>=0 ; k++)
      avg += wrk[r-k][vsum];
   avg /= k;
//Print("el valor de k :::> ",k);
   wrk[r][avolty] = avg;
   if(wrk[r][avolty] > 0)
      double dVolty = wrk[r][volty]/wrk[r][avolty];
   else
      dVolty = 0;
...
//Print("dVolty: ",dVolty,", Kv: ",Kv,", len2: ",len2,", pow2: ",pow2);

   if(del1 > 0)
      wrk[r][bsmax] = hprice;
   else
      wrk[r][bsmax] = hprice - Kv*del1;
   if(del2 < 0)
      wrk[r][bsmin] = lprice;
   else
      wrk[r][bsmin] = lprice - Kv*del2;
...
//if(r<=25)
//Print("candle: ",TimeToStr(Time[Bars-r-1]),",wrk[r][bsmax]: ",wrk[r][bsmax],
//", wrk[r][bsmin]: ",wrk[r][bsmin],", wrk[r][volty]: ",wrk[r][volty]*100,
//", wrk[r][vsum]: ",wrk[r][vsum],", lprice: ",lprice*100,
//      ", hprice: ",hprice*100,", del1: ",del1*10000,", del2: ",del2*10000,", dnValue: ",dnValue*100,
//      ", upValue: ",upValue*100,", miValue: ",miValue*100,", Kv: ",Kv*10000,", r: ",r);
  }
      ...
      if(alertsOnZeroCross)
        {
         if(trend[whichBar] != trend[whichBar+1])
           {
            if(trend[whichBar] ==  1)
               doAlert(alertTime1,alertWhat1,whichBar,"price crossed zero (middle) line up");
            if(trend[whichBar] == -1)
               doAlert(alertTime1,alertWhat1,whichBar,"price crossed zero (middle) line down");
           }
        }
      else
        {
         if(alertsOnAllBreakOutBars)
            ...
            condition = (trend[whichBar] != trend[whichBar+1]);
         if(condition)
           {
            if(trend[whichBar] ==  1)
               doAlert(alertTime1,alertWhat1,whichBar,"price broke upper band");
            if(trend[whichBar] == -1)
               doAlert(alertTime1,alertWhat1,whichBar,"price broke lower band");
           }
        }
     }
  }
   ...
   if(previousAlert != doWhat || previousTime != Time[forBar])
     {
      ...
      if(alertsMessage)
         Alert(message);
      if(alertsEmail)
         SendMail(StringConcatenate(Symbol(),"jurik volty bands"),message);
      if(alertsSound)
